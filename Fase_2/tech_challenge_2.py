...
df['vol_historica_rolling_anual'] = df['retorno_diario'].rolling(window=252).std().fillna(0) * (252**0.5)
df['vol_historica_rolling_anual'] = df['vol_historica_rolling_anual'].round(4)
df['vol_historica_nao_linear'] = df['vol_21d'] * (252**0.5)
df['vol_historica_nao_linear'] = df['vol_historica_nao_linear'].round(4)
df['vol_historica_linear'] = round(df['retorno_diario'].std(), 4)
df.head(256)

df['Fechamento'] = df['Último'].apply(lambda x: str(x).replace('.', '').replace(',', '.')).astype(float)
...
df_2['vol_historica_rolling_anual'] = df_2['retorno_diario'].rolling(window=252).std().fillna(0) * (252**0.5)
df_2['vol_historica_rolling_anual'] = df_2['vol_historica_rolling_anual'].round(4)
df_2['vol_historica_nao_linear'] = df_2['vol_21d'] * (252**0.5)
df_2['vol_historica_nao_linear'] = df_2['vol_historica_nao_linear'].round(4)
df_2['vol_historica_linear'] = round(df_2['retorno_diario'].std(), 4)
df_2.head(256)

df_2['Fechamento'] = df_2['Último'].apply(lambda x: str(x).replace('.', '').replace(',', '.')).astype(float)
correlacao_2 = df_2['Último'].corr(df['Vol.'])
df_2['correlacao_fechamento_volume'] = correlacao_2
